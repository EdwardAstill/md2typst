...
+ *Static FEA* (Abaqus) -- find where the pipe touches the seabed and where it spans freely
+ *Modal analysis* (Abaqus) -- get natural frequencies and mode shapes of the free spans
+ *Fatigue calc* -- VIV response models + Palmgren-Miner rule = fatigue life estimate
+ *Sensitivity study* -- wiggle key parameters, see what matters
...
Knee point (where segments meet): S\_knee \~ 74.1 MPa, N\_knee \~ 10^6 cycles. These parameters are read off Figure 2-2 and match a DNV E-class curve in seawater with cathodic protection. The exact values aren't given -- we extracted them from the figure, so justify in the report.
...
#table(
  ...
  [Internal pressure],
  [10 MPa],
  [Typical operating pressure for subsea oil/gas pipeline],
  [Temperature rise],
  [50 degC],
  [Hot product relative to ambient seawater temperature],
  [Lay tension],
  [500 kN],
  [Residual axial tension from installation (S-lay or J-lay)],
  [Pipe contents density],
  [1025 kg/m^3],
  [Assumed water-filled (conservative for submerged weight)],
  [Seawater density],
  [1025 kg/m^3],
  [Standard value],
  [Strouhal number],
  [0.2],
  [Standard value for circular cylinder in sub-critical flow],
  [Element type],
  [PIPE31],
  [2-node linear pipe beam element (appropriate for slender pipeline)],
  [Element size],
  [1.0 m],
  [Confirmed by mesh convergence study],
  [Material model],
  [Linear elastic],
  [Stress levels well below yield; fatigue is in elastic range],
  [End boundary conditions],
  [Pinned at start, roller at end],
  [Pipeline continues beyond modelled section],
)
...
+ Open Abaqus/CAE
+ Create a 3D Deformable Wire part
+ The wire follows the seabed bathymetry: for each KP point in the CSV, plot a point at (x, y) where x = KP (raw value from CSV, so starts at 2000) and y = -(Depth) + OD/2. The OD/2 offset puts the pipe axis just above the seabed surface
+ Connect the points with straight line segments to form a single wire


=== 4.2 Define the Material and Section

+ Create a material: E = 207 GPa, v = 0.3, density = 7861 kg/m^3, thermal expansion = 1.3e-5 /degC
+ Create a Pipe Profile: outer radius r = 0.184 m, wall thickness t = 0.024 m
+ Create a Beam Section using the pipe profile and material
+ Assign the section to the wire part


=== 4.3 Mesh the Part

+ Seed the part with element size = 1.0 m (for convergence study, also try 2.0, 0.5, 0.25 m)
+ Assign element type: PIPE31 (2-node linear pipe element in 3D space)
+ Generate mesh
...
+ Create node sets: StartNode (first node), EndNode (last node), AllNodes (all nodes)


=== 4.5 Define the Step

+ Create a Static, General step
+ Enable *geometric nonlinearity* (Nlgeom = ON) -- this is important because the pipe undergoes large displacements as it settles onto the seabed
+ Set increment sizes: initial 0.1, minimum 1e-10, maximum 0.5, max increments 1000


=== 4.6 Apply Boundary Conditions

+ *StartNode*: pin -- fix translations only (U1=U2=U3=0), leave rotations free
+ *EndNode*: roller (U2=U3=0, free in U1 to allow axial expansion)
+ *AllNodes*: constrain U3=0 (keep pipe in the x-y plane -- we model in 2D effectively)


=== 4.7 Apply Loads

+ *Gravity*: acceleration of 9.81 m/s^2 in the -y direction, applied to the whole model. The pipe density (7861 kg/m^3) gives the steel weight. For the contents, we need to account for the internal fluid weight and external buoyancy. This is handled through an effective density or by adding the content weight and subtracting buoyancy separately
+ *Thermal load*: predefined temperature field of 50 degC applied to all nodes (this creates compressive axial force in the restrained pipe)
+ *Lay tension*: concentrated axial force of 500 kN at the EndNode (or distributed)
+ *Internal pressure*: 10 MPa applied as pipe pressure load
+ *External pressure*: hydrostatic pressure = rho\_seawater \* g \* depth at each point
...
- The spring stiffness = K\_static \* element\_length = 200 kN/m^2 \* 1.0 m = 200 kN/m per spring
...
+ Submit the job (Job name: StaticAnalysis)
+ Check that the job completes without errors
+ In the Visualization module, check:
...
+ Start from the completed static model
+ *Change spring stiffnesses* from 200 kN/m^2 (static) to 1900 kN/m^2 (dynamic). The dynamic stiffness is higher because during vibration the soil responds more stiffly. Scale the nonlinear spring table by the ratio 1900/200 = 9.5x
+ Add a new step: *Frequency* (linear perturbation)
...
+ Submit as a new job (ModalAnalysis)
+ In the Visualization module, check:
...
where U = current velocity (m/s), f\_n = natural frequency of the mode (Hz), D = OD = 0.368 m.

*Step B: Check VIV Onset*

Cross-flow VIV occurs when 3.0 \< V\_R \< 8.0 (peak response at V\_R = 5.0, max A/D = 0.9). In-line VIV occurs when 1.0 \< V\_R \< 3.5 (peak response at V\_R = 2.2, max A/D = 0.15).

If V\_R is outside both ranges, there is no VIV for this combination -- skip it.

*Step C: Get VIV Amplitude*

Triangular response model:

- Linear ramp from onset to peak V\_R
- Linear ramp from peak to cessation V\_R
...
Z = section modulus = I / (OD/2), M\_max = peak bending moment in the span for that mode.
...
#table(
  ...
  [*Variation*],
  [*Why*],
  [Soil stiffness (K\_static, K\_dynamic)],
  [+/- 50%],
  [Soil properties are uncertain; affects span lengths and frequencies],
  [Current velocity],
  [+/- 20%],
  [Current data is approximate; directly affects VIV excitation],
  [Lay tension],
  [+/- 50%],
  [Affects effective axial force, which changes span behaviour],
  [Temperature rise],
  [+/- 20%],
  [Affects thermal axial force],
  [Damping],
  [Vary stability parameter],
  [Affects VIV amplitude],
  [Element size],
  [2.0, 1.0, 0.5, 0.25 m],
  [Mesh convergence -- proves solution is mesh-independent],
)
...
+ *Mesh convergence*: run with element sizes 2.0, 1.0, 0.5, 0.25 m. Plot natural frequency (or max stress) vs element size. Show it converges. Table + plot.
+ *Analytical benchmark*: for a simple span, compare Abaqus frequency against the beam-on-elastic-foundation formula:
```
f_n = (n*pi/L)^2 * sqrt(EI / (rho*A)) / (2*pi)
```

  Sanity check that the FE model isn't broken.
+ *Equilibrium check*: sum of spring reactions should equal total pipe weight minus buoyancy.
+ *Sniff test*: does the fatigue life look reasonable? Should be in the years-to-decades range for a real pipeline.
...
VIV only happens within certain V\_R ranges (see Section 7.2). Outside those, no lock-in, no significant vibration.
...
n\_i = applied cycles at stress range S\_i, N\_i = allowable cycles from S-N curve. D hits 1.0 = failure. Fatigue life = 1/D years (if D is per year).
...
+ *Compression-only springs* -- soil only pushes up, that's a nonlinearity
+ *Geometric nonlinearity* -- pipe displaces significantly as it settles, and axial force interacts with deflection (P-delta effect)
...
+ Figure of deformed pipe on seabed with free spans marked
+ Table of free span locations (KP range, length, gap height)
+ Table of natural frequencies per span and mode
+ Mode shape plots for the critical spans
+ Mesh convergence plot
+ Table of fatigue damage per span/mode/current bin
+ Total fatigue life in years
+ Sensitivity plots
+ Which span is critical (governs fatigue life)
...
+ Bai, Q. and Bai, Y. (2014) _Subsea Pipeline Design, Analysis, and Installation_, Elsevier. Chapter 14.
+ Guo, B. et al. (2013) _Offshore Pipelines: Design, Installation, and Maintenance_, Elsevier. Chapter 5.
+ Sollund, H. and Vedeld, K. (2014) _A Finite Element Solver for Modal Analysis of Multi-Span Offshore Pipelines_, Research Report No. 2014-01, University of Oslo. Section 4.
+ DNV (2006) _Recommended Practice DNV-RP-F105: Free Spanning Pipelines_, Det Norske Veritas.
+ Dassault Systemes, _Abaqus Analysis User's Manual_.
